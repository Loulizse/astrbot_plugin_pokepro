...
class PokeproPlugin(Star):
    ...
    async def on_poke_cmd(self, event: AiocqhttpMessageEvent):
        ...
        if self.special_qq in target_ids:
            # 不执行戳，改为戳命令发送者
            ...
            # 发送表情包给命令发送者
            img_path = self.cfg.get_image()  # 获取随机表情包
            if img_path:
                chain = event.image_result(img_path)
                await self.context.send_message(event.unified_msg_origin, chain)
            # 发送文本消息
            text_chain = event.plain_result("不准戳我的主人，哈！")
            await self.context.send_message(event.unified_msg_origin, text_chain)
            event.stop_event()
            return
        if "我" in msg:
